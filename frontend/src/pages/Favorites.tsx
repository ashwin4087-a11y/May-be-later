...
export default function Favorites() {
  ...
  const handleToggleFavorite = async (id: string, nextState: boolean) => {
    // If it was unfavorited from the grid directly, immediately remove it from the view
    if (!nextState) {
  ...
  const handleDownloadFavorites = async () => {
    ...
    setError(null);
    try {
      await downloadScreenshotsAsZip(screenshots, 'Maybe Later - Favorites.zip', setDownloadProgress);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to download favorites.');
    } finally {
